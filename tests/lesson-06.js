...
async function run() {
  ...
  await test('El servidor está corriendo en el puerto 3000', async () => {
    try {
  ...
  await test('POST /books — crea un libro con estado 201', async () => {
  ...
  await test('GET /books — devuelve un arreglo de libros', async () => {
    const res = await fetch(`${BASE}/books`);
  ...
  await test('GET /books/:id — devuelve un libro por su ID', async () => {
  ...
  await test('PATCH /books/:id — actualiza el título', async () => {
    if (!bookId) throw new Skip('primero debe pasar POST /books');
    const res = await fetch(`${BASE}/books/${bookId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  await test('DELETE /books/:id — elimina un libro', async () => {
    if (!bookId) throw new Skip('primero debe pasar POST /books');
